...
void CNN::rectangleThread(const cv::Mat cvImg, std::vector<TargetBox>& boxes, const char* class_names[])
{
	while (true)
	{
		CNN::rectangle(cvImg, boxes, class_names);
	}
}

void CNN::rectangleT(const cv::Mat cvImg, std::vector<TargetBox>& boxes, const char* class_names[])
{
	//std::thread t1(&CNN::rectangleThread(cvImg, boxes, class_names));
	std::thread t1([&](){CNN::rectangleThread(cvImg, boxes, class_names);});
	t1.detach();
